...
supported_statistics = ["mapped", "reads_correct", "reads_incorrect", "accuracy_percent", "mapq60", "wrong_mapq60", "softclips", "clipped_or_unmapped", "match_bp", "speed", 
...
rule stats_tsv:
    ...
    If you want to add a new supported statistic:
    1. add it to supported_statistics at the top of the snakemake
    2. make sure that the file it's stored in is generated by a rule and includes it 
    ...
    run:
        ...
        if "match_bp" in STATS_INC:
            match_bp = int(re.search(r"Matches:\s+([\d.]+)\s", text).group(1))
            df["match_bp"] = match_bp
        # =======================
        # From vg gamcompare
        # =======================
        ...
        # softclips are used to calculate other stats, so always grab it just in case
